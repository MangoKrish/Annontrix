...
// FLIP filtering · capability cross-links · plates ·
// copy email · project brief (mailto)
// ==========================================================
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- Work index: filtering (category + stack) with FLIP ---
  const items = Array.from(document.querySelectorAll('#workIndex .work-item'));
  ...
  // --- Capabilities: live counts + cross-link into the index ---
  ...
  // --- Service rows link into a filtered index ("#work=web" etc.) ---
  function applyHash(h, updateHash) {
    if (h.startsWith('work=stack:')) {
      ...
      if (btn && !btn.disabled) filterByStack(token, btn.firstChild.textContent.trim(), updateHash);
      return true;
    }
    if (h.startsWith('work=')) {
      const cat = h.slice('work='.length);
      if (chips.some((c) => c.dataset.filter === cat)) filterByCategory(cat, updateHash);
      return true;
    }
    if (h.startsWith('p-') || h.startsWith('s-')) {
  ...
  document.querySelectorAll('a[href^="#work="]').forEach((a) => {
    a.addEventListener('click', (e) => {
      e.preventDefault();
      applyHash(a.getAttribute('href').slice(1), true);
      document.getElementById('work').scrollIntoView({ behavior: reducedMotion ? 'auto' : 'smooth' });
    });
  });

  // --- Restore filter / open row from URL hash ---
